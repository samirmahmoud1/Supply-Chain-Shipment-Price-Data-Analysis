...
@st.cache_data
def load_data():
    # غير الاسم لو ملفك في مكان مختلف
    ...
    # الاحتفاظ بالصفوف اللي فيها مواعيد أساسية
    ...
    # تنظيف الوزن: إزالة <= 0 ثم قصّ عند 95th percentile
...
with tab4:
    ...
    if not monthly_stats.empty:
        ...
        ax.plot(monthly_stats["year_month"], monthly_stats["total_quantity"], marker="o")
        ax.set_xlabel("Month")
        ax.set_ylabel("Total Quantity")
        ax.set_title("Total Quantity Over Time")
        plt.xticks(rotation=45, ha="right")
        st.pyplot(fig)
    else:
        st.write("No data after applying filters.")
